...
public class Order implements BaseModel<Order> {
	...
	public static Order inflate(JSONObject object) {
		Order order = new Order((String) null);
		...
		if (object.has(Api.ADDRESS)) {
			try {
				order.address = Address.inflate(object
						.getJSONObject(Api.ADDRESS));
			} catch (JSONException e) {
				// Nothing to be done
			}
		}
		if (object.has(Api.PAYMENT_CARD)) {
			try {
				order.card = PaymentCard.inflate(object
						.getJSONObject(Api.PAYMENT_CARD));
			} catch (JSONException e) {
				// Nothing to be done
			}
		}
	...
	public JSONObject toJson() throws JSONException {
		...
		object.put(Api.TRACKER, tracker);
		return object;
	}

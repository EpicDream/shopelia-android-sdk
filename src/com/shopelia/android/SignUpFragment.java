...
public class SignUpFragment extends ShopeliaFragment<OnSignUpListener> {
	...
	static {
		...
		EVENTS.put(R.id.payment_card,
				Analytics.Properties.Steps.SigningUp.PAYMENT_CARD);
		...
		CLICK_ON.put(R.id.address,
				Analytics.Properties.ClickOn.SigningUp.ADDRESS);
		CLICK_ON.put(R.id.payment_card,
				Analytics.Properties.ClickOn.SigningUp.PAYMENT_CARD);
	}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		return inflater.inflate(R.layout.shopelia_signup_fragment, container,
				false);
	}
	...
	public void onCreateAccountError(JSONObject object) {
		...
		for (int index = 0; index < count; index++) {
			...
			FormField field = mFormContainer.findFieldByPath(Order.Api.USER,
					User.Api.EMAIL);
			if (field != null) {
				field.setError(errors.optString(0));
			} else {
				Toast.makeText(getActivity(), errors.optString(0),
	...
	protected void onCreateShopeliaActionBar(ActionBar actionBar) {
		...
		actionBar.clear();
		actionBar.addItem(new TextButtonItem(R.id.shopelia_action_bar_login,
				getActivity(), R.string.shopelia_action_bar_sign_in));
		actionBar.commit();
	}
	...
	private OnValidateListener mPhoneOnValidateListener = new OnValidateListener() {
		...
		@Override
		public boolean onValidate(EditTextField editTextField,
		...
		public void afterTextChanged(Editable s) {
			...
			if (PhoneField.PHONE_PATTERN.matcher(number).matches()
					&& !number.equals(mCurrentNumber)) {
				mCurrentNumber = number;
				ShopeliaRestClient.get(
						Command.V1.Phones.Lookup(mCurrentNumber), null,
						new AsyncCallback() {
							...
							public void onComplete(HttpResponse httpResponse) {
								try {
									...
									AddressField field = (AddressField) mFormContainer
											.findFieldById(R.id.address);
									if (field != null) {
	...
	private OnValidateListener mEmailOnValidateListener = new OnValidateListener() {
		...
		public void afterTextChanged(Editable s) {
			super.afterTextChanged(s);
			final EmailField emailField = mFormContainer
					.findFieldById(R.id.email);
			if (emailField.onValidation(false)) {
				...
				ShopeliaRestClient.post(Command.V1.Users.Exists(), params,
						new AsyncCallback() {
							...
							public void onComplete(HttpResponse httpResponse) {
								if (httpResponse.getStatus() == 204
									...
									arguments.putString(
											SignInFragment.ARGS_EMAIL, email);
									getContract().requestSignIn(arguments);
									emailField
											.setError(getString(
													R.string.shopelia_error_user_already_exists,
													email));
									Toast.makeText(
											getActivity(),
											getString(
		...
		public void onValidChanged(FormField field) {
			...
			if (event != null && field.isValid() && field.getResult() != null) {
				track(Analytics.Events.Steps.SignUp.SIGNING_UP,
						AnalyticsBuilder.prepareStepPackage(getActivity(),
								event));
			}
		};
		...
		public void onClick(View v) {
			...
			if (action != null) {
				track(Analytics.Events.Steps.SignUp.SIGN_UP_ACTION,
						AnalyticsBuilder.prepareClickOnPackage(getActivity(),
								action));
			}
		}
	};

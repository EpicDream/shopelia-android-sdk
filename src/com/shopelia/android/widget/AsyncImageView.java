...
import com.shopelia.android.image.ImagePostTreatment;
...
public class AsyncImageView extends ImageView {
	...
	private static final Drawable EMPTY_DRAWABLE = new ColorDrawable(
			Color.TRANSPARENT);
	...
	private ImagePostTreatment mPostTreatment;
	...
	public AsyncImageView(Context context, AttributeSet attrs, int defStyle) {
		...
		Drawable d = a
				.getDrawable(R.styleable.AsyncImageView_shopelia_defaultSrc);
		if (d != null) {
			setDefaultImageDrawable(d);
		}

		final int inDensity = a.getInt(
				R.styleable.AsyncImageView_shopelia_inDensity, -1);
		if (inDensity != -1) {
			setInDensity(inDensity);
		}

		setCrossFadingEnabled(a.getBoolean(
				R.styleable.AsyncImageView_shopelia_crossFadingEnabled,
				mIsCrossFadingEnabled));
		setCrossFadingDuration(a.getInt(
				R.styleable.AsyncImageView_shopelia_crossFadingDuration,
				mCrossFadingDuration));
		setUrl(a.getString(R.styleable.AsyncImageView_shopelia_url));
		mIsFixedSize = a.getBoolean(
				R.styleable.AsyncImageView_shopelia_isFixedSize, false);
	...
	public boolean reload() {
		if (mRequest == null && mUrl != null) {
			...
			mRequest.postTreatment = mPostTreatment;
			mIsLoading = true;
	 ...
	 */
	public void setOnAsyncImageViewLoadListener(
			OnAsyncImageViewLoadListener listener) {
	 ...
	 *            AsyncImageView to display the default image
	 */
	public boolean setUrl(String url, ImagePostTreatment treatment) {
		mAttempts = 0;
		// Check the url has changed
		if (url != null && url.equals(mUrl)
				&& (mIsLoading || (!mIsLoading && mBitmap != null))) {
			return true;
		}
		mIsLoading = true;
		stopLoading();
		mUrl = url;
		mPostTreatment = treatment;
	...
	private ImageRequest.Callback mImageRequestCallback = new ImageRequest.Callback() {
		...
		public void onImageRequestStarted(ImageRequest request) {
			if (mOnAsyncImageViewLoadListener != null) {
				mOnAsyncImageViewLoadListener
						.onLoadingStarted(AsyncImageView.this);
			}
		...
		public void onImageRequestFailed(ImageRequest request,
			...
			if (mOnAsyncImageViewLoadListener != null) {
				mOnAsyncImageViewLoadListener.onLoadingFailed(
						AsyncImageView.this, exception);
			}
		}
		...
		public void onImageRequestEnded(ImageRequest request, Bitmap image) {
			...
			if (canCrossFade()) {
				final TransitionDrawable d = new TransitionDrawable(
						new Drawable[] {
								mDefaultDrawable == null ? EMPTY_DRAWABLE
										: new KeepRatioDrawable(
												mDefaultDrawable, mAlign),
								new KeepRatioDrawable(new BitmapDrawable(
										getResources(), image), mAlign) });
				setImageDrawable(d);
			...
			if (mOnAsyncImageViewLoadListener != null) {
				mOnAsyncImageViewLoadListener.onLoadingEnded(
						AsyncImageView.this, image);
			}
		}
	...
	private static class KeepRatioDrawable extends Drawable {
		...
		protected void onBoundsChange(Rect bounds) {
			...
			if (width > 0 && height > 0) {
				...
				final float scale = Math.min((float) containerWidth
						/ (float) width, (float) containerHeight
						/ (float) height);
				...
				mDrawable.setBounds((int) dx, (int) dy,
						(int) (scaledWidth + dx), (int) (scaledHeight + dy));
			} else {

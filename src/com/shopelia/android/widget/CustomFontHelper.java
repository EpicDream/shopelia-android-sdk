...
final class CustomFontHelper {
	...
	private static final SparseArray<SparseArray<Typeface>> sTypefaces = new SparseArray<SparseArray<Typeface>>(
			3);
	...
	static {
		// Add fonts here
		FONTS = new Font[] {
				new Font(FAMILY_NORMAL, STYLE_NORMAL,
						R.raw.shopelia_helvetica_neue),
				new Font(FAMILY_NORMAL, STYLE_BOLD,
						R.raw.shopelia_helvetica_neue_bold),
				new Font(FAMILY_LIGHT, STYLE_NORMAL,
						R.raw.shopelia_helvetica_light),
				new Font(FAMILY_LIGHT, STYLE_ITALIC,
						R.raw.shopelia_helvetica_light_italic) };
	}
	 ...
	 * @return
	 */
	public static Typeface getTypeface(Context context, int fontFamily,
			int fontStyle) {

		if (android.os.Build.VERSION.SDK_INT < android.os.Build.VERSION_CODES.GINGERBREAD) {
			return fontStyle == STYLE_BOLD ? Typeface.DEFAULT_BOLD
					: Typeface.DEFAULT;
		}
		...
		if (bestMatchFont != null) {
			typeface = tryCreateTypefaceFromSharedDirectory(context,
					bestMatchFont.getFilename(), bestMatchFont.getResId());
		}
	...
	public static Typeface tryCreateTypefaceFromSharedDirectory(
			Context context, String path, int resId) {
		File fontDir = new File(Environment.getExternalStorageDirectory(),
				Config.PUBLIC_FONTS_DIRECTORY);
		fontDir.mkdirs();
	...
	private static class Font {
		...
		public int match(int family, int style) {
			return 2 * matchIntegers(family, mFamily)
					+ matchIntegers(style, mStyle);
		}

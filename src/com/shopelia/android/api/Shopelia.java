...
public final class Shopelia implements Parcelable {
	...
	public interface OnProductAvailabilityChangeListener {
		 ...
		 * @param newStatus
		 *            The new status may be one of the Shopelia.STATUS_*
		 *            integers.
		 */
		public void onProductAvailabilityChanged(Shopelia shopelia,
				int newStatus);
	}
	 ...
	 */
	public static final String EXTRA_DISPLAY_WELCOME_SCREEN = Config.EXTRA_PREFIX
			+ "DISPLAY_WELCOME_SCREEN";

	public static final String EXTRA_TRACKER = ShopeliaActivity.EXTRA_TRACKER;
	...
	public Shopelia(Context context, String productUrl, String trackerName,
			OnProductAvailabilityChangeListener l) {
		setOnProductAvailabilityChangeListener(l);
		mTrackerName = trackerName;
		mTracker = Tracker.Factory.getTracker(Tracker.PROVIDER_SHOPELIA,
				context);
		mData = new Intent();
	...
	void setStatus(final int status) {
		if (status != mStatus) {
			...
			if (mOnProductAvailabilityChangeListener != null) {
				mOnProductAvailabilityChangeListener
						.onProductAvailabilityChanged(this, status);
			}
		}
	}
	 ...
	 *            The product URL
	 * @param data
	 *            Extra data or null if unnecessary
	 */
	private final void checkout(Context context, String productUrl,
			Intent data, int requestCode) {
		notifyView();
		if (data == null) {
			data = new Intent();
		}
		...
		data.putExtra(ShopeliaActivity.EXTRA_STYLE,
				ShopeliaActivity.STYLE_DIALOG);
		mTracker.onClickShopeliaButton(getProductUrl(), mTrackerName);
		mTracker.flush();
		data.putExtra(EXTRA_TRACKER, mTrackerName);
	 ...
	 * 
	 * @param display
	 *            True for displaying the screen (default behaviour)
	 */

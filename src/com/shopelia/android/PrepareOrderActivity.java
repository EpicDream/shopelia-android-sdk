...
public class PrepareOrderActivity extends AccountAuthenticatorShopeliaActivity
		implements OnSignUpListener, OnSignInListener {

	/**
	 * An email that will be pre-filled for the user
	 */
	public static final String EXTRA_USER_EMAIL = Config.EXTRA_PREFIX
			+ "USER_EMAIL";

	/**
	 * An phone that will be pre-filled for the user
	 */
	public static final String EXTRA_USER_PHONE = Config.EXTRA_PREFIX
			+ "USER_PHONE";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (isCalledByAcountManager()
				&& UserManager.get(this).getAccount() != null) {
			Toast.makeText(this, R.string.shopelia_account_only_one,
			...
			if (!UserManager.get(this).isLogged() || isCalledByAcountManager()) {
				...
				if (UserManager.get(this).getLoginsCount() > 0) {
					ft.add(R.id.fragment_container, mSignInFragment,
							mSignInFragment.getName());
				} else {
					ft.add(R.id.fragment_container, mSignUpFragment,
							mSignUpFragment.getName());
				}
				ft.commit();
			} else {
		...
		if (isCalledByAcountManager()) {
			...
			((LinearLayout) findViewById(R.id.main_form))
	...
	protected void onResume() {
		super.onResume();
		((ProductSheetWidget) findViewById(R.id.product_sheet)).setProductInfo(
				getOrder().product, false);
	}

	public void onEventMainThread(OnProductNotAvailable event) {
		ProductNotFoundFragment fragment = ProductNotFoundFragment
				.newInstance(event.resource);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case REQUEST_ADD_PAYMENT_CARD:
			if (resultCode == RESULT_OK) {
				getOrder().user
						.addPaymentCard((PaymentCard) data
		...
		if (!UserManager.get(this).isLogged() && !isFinishing()) {
			if (fragment == null) {
				...
				ft.replace(R.id.fragment_container,
						resultCode == RESULT_LOGOUT ? mSignInFragment
								: mSignUpFragment);
				ft.commit();
			}
		}
	...
	private void createAccount() {
		...
		api.register(new Object() {
			public void onEventMainThread(OnAccountCreationSucceedEvent event) {
				...
				if (isCalledByAcountManager()) {
					...
					if (order.user.paymentCards.size() == 0) {
						Intent intent = new Intent(PrepareOrderActivity.this,
								AddPaymentCardActivity.class);
						intent.putExtra(AddPaymentCardActivity.EXTRA_REQUIRED,
								true);
			...
			public void onEventMainThread(OnApiErrorEvent event) {
				...
				if (event.exception != null) {
					Toast.makeText(PrepareOrderActivity.this,
							R.string.shopelia_error_network_error,
							Toast.LENGTH_LONG).show();
				} else {
		...
		api.createAccount(getTrackerName(), order.user, order.address,
				order.card);
	}
	...
	public void onSignIn(JSONObject result) {
		...
		api.register(new Object() {
			...
			public void onEventMainThread(OnApiErrorEvent event) {
				...
				if (event.json != null) {
					Toast.makeText(PrepareOrderActivity.this,
							event.json.optString(ErrorInflater.Api.ERROR),
							Toast.LENGTH_SHORT).show();
				}
			}
	...
	public ValidationButton getValidationButton() {
		return (ValidationButton) findViewById(R.id.footer).findViewById(
				R.id.validate);
	}

	@SuppressLint("NewApi")
	private void switchFragments(final ShopeliaFragment<?> inFragment,
			final ShopeliaFragment<?> outFragment) {
		...
		fadeOut.setAnimationListener(new AnimationListener() {
			...
			public void onAnimationEnd(Animation animation) {
				...
				final ResizeAnimation resize = new ResizeAnimation(container,
		...
		imm.hideSoftInputFromWindow(findViewById(R.id.fragment_container)
				.getWindowToken(), 0);
	}

	public void switchFragmentsNow(final ShopeliaFragment<?> inFragment,
			final ShopeliaFragment<?> outFragment, boolean forceExecute) {
		FragmentManager fm = getSupportFragmentManager();
		FragmentTransaction ft = getSupportFragmentManager().beginTransaction();
		try {
			mSavedStates.put(outFragment.getClass(),
					fm.saveFragmentInstanceState(outFragment));
		} catch (Exception e) {
		...
		if (mSavedStates.containsKey(inFragment.getClass())) {
			inFragment.setInitialSavedState(mSavedStates.get(inFragment
					.getClass()));
		}

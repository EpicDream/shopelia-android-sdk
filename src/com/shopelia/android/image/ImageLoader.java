...
public class ImageLoader {
	...
	//
	private static final long DISK_ENTITY_TIME_TO_LIVE = 7 * 24 * 60 * 60
			* 1000;
	...
	private ImageLoader(Context context) {
		...
		mHardMemoryCache = new LruCache<String, Bitmap>(
				(int) (activityManager.getMemoryClass() / 8f * 1024f * 1024f)) {
			@Override
		...
		mMaximumMemoryCachedImageSize = 4 * metrics.widthPixels
	...
	public void override(String url, Bitmap bitmap) {
		try {
			FileOutputStream out = new FileOutputStream(mCache.obtain(url));
	...
	public boolean loadRequest(Context context, ImageRequest request) {
		...
		if (TextUtils.isEmpty(url)) {
			...
			if (callback != null) {
				...
				callback.onImageRequestFailed(request,
		...
		if (url.startsWith(ASSETS_PREFIX)) {
			if (bitmap == null) {
				...
				InputStream is = null;
				try {
					is = mAssetManager.open(url
							.replace(ASSETS_PREFIX, "icons/"));
		...
		final WorkerHandler workerHandler = mIsPaused ? null
				: getWorkerHandler();
	...
	public void setPaused(boolean paused) {
		if (mIsPaused != paused) {
			...
			if (paused) {
				...
				WorkerHandler handler;
				while ((handler = getWorkerHandler()) != null
						&& !mRequestsQueue.isEmpty()) {
	...
	private WorkerHandler getWorkerHandler() {
		if (mWorkers == null) {
			...
			for (int i = 0; i < WORKER_POOL_COUNT; i++) {
				...
				record.worker = new HandlerThread(LOG_TAG + " worker#" + i,
	...
	private void sendRequestToWorker(ImageRequest request,
	...
	private void postQuitWorkersIfNecessary() {
		if (canQuitWorkers()) {
			mHandler.removeMessages(MESSAGE_ON_QUIT);
			mHandler.sendEmptyMessageDelayed(MESSAGE_ON_QUIT,
					KEEP_ALIVE_DURATION);
		}
	}
	...
	private ImageRequest removeLast() {
		ListIterator<ImageRequest> it = mRequestsQueue
				.listIterator(mRequestsQueue.size());
	...
	/* package */class WorkerHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case MESSAGE_LOAD:
				...
				if ((request.cachePolicy & ImageRequest.CACHE_POLICY_DISK) != 0) {
					try {
						if (mCache.exists(request.url)) {
							long creationDate = mCache
									.getCreationDate(request.url);
							if (System.currentTimeMillis() - creationDate < DISK_ENTITY_TIME_TO_LIVE) {
								diskBitmap = BitmapFactory
										.decodeStream(new FileInputStream(
												mCache.load(request.url)));
							}
						}
						...
						if (Config.ERROR_LOGS_ENABLED) {
							Log.e(LOG_TAG,
									"Unable to retrieve the on-disk cached image",
									e);
						}
					} finally {
				...
				if (diskBitmap == null
						&& (request.cachePolicy & ImageRequest.CACHE_POLICY_NETWORK) != 0) {
					...
					while (request.retryCount <= DEFAULT_RETRY_COUNT) {
						...
						try {
							iStream = new BufferedInputStream(new URL(
						...
						try {
							input = tempSuccess ? new FlushedInputStream(
									new FileInputStream(file))
									: new FlushedInputStream(new URL(
											request.url).openStream());
							networkBitmap = BitmapFactory.decodeStream(input,
									null, null);
							if (request.postTreatment != null) {
								networkBitmap = request.postTreatment
										.compute(networkBitmap);
								try {
									oStream = new BufferedOutputStream(
											new FileOutputStream(file));
									networkBitmap.compress(CompressFormat.PNG,
											100, oStream);
								} catch (Exception e) {
				...
				// completed
				final Bitmap bitmap = networkBitmap != null ? networkBitmap
						: diskBitmap;
		...
		public void handleMessage(Message msg) {
			...
			case MESSAGE_ON_END:
				...
				if (callback != null && !request.isCancelled) {
					callback.onImageRequestEnded(request,
							(Bitmap) request.result);
				}

...
class ShopeliaTracker extends Tracker {
	...
	interface FlushDelegate {
		public boolean send(String uuid, String tracker, String action,
				ArrayList<ShopeliaEvent> events);
	}
	...
	private static final String DEFAULT_TRACKER_NAME = Config.DEFAULT_TRACKER;
	...
	private WeakReference<Context> mApplicationContext = new WeakReference<Context>(
			null);
	private ScheduledTask mFlushTask = new ScheduledTask(new Handler(
	...
	public static ShopeliaTracker getInstance() {
		return sInstance == null ? sInstance = new ShopeliaTracker()
				: sInstance;
	}
			...
			public void onReceiveUuid(final String uuid) {
				mFlushExecutor.execute(new Runnable() {
					...
					public void run() {
						...
						for (Map.Entry<String, ShopeliaEvent[]> event : events
								.entrySet()) {
							List<ShopeliaEvent> notSent = flush(uuid,
									event.getKey(), event.getValue());
							// Update sent entries
							synchronized (mEvents) {
								HashSet<ShopeliaEvent> e = mEvents.get(event
										.getKey());
	...
	private Map<String, ShopeliaEvent[]> getReadOnlyVersion(
			Map<String, HashSet<ShopeliaEvent>> events) {
		Map<String, ShopeliaEvent[]> out = new HashMap<String, ShopeliaEvent[]>(
				events.size());
		synchronized (events) {
			for (Entry<String, HashSet<ShopeliaEvent>> event : events
					.entrySet()) {
				ShopeliaEvent[] entries = new ShopeliaEvent[event.getValue()
						.size()];
	...
	private void save() {
		...
		if (context != null && mEvents != null) {
			SharedPreferences.Editor editor = context.getSharedPreferences(
					PRIVATE_PREFERENCE, Context.MODE_PRIVATE).edit();
			synchronized (mEvents) {
				try {
					JSONObject eventsObject = new JSONObject();
					for (Map.Entry<String, HashSet<ShopeliaEvent>> entry : mEvents
							.entrySet()) {
						if (entry.getKey() != null) {
							JSONArray entryArray = new JSONArray();
							final long now = System.currentTimeMillis();
							for (ShopeliaEvent event : entry.getValue()) {
								if (event.sent_at + mExpiryDelay >= now
	...
	private void load(Context context) {
		...
		SharedPreferences preferences = context.getSharedPreferences(
				PRIVATE_PREFERENCE, Context.MODE_PRIVATE);
		if (preferences.contains(PREFS_VERSION)
				&& preferences.contains(PREFS_EVENTS)) {
			synchronized (mEvents) {
				int version = preferences
						.getInt(PREFS_VERSION, CURRENT_VERSION);
				try {
					JSONObject eventsObject = new JSONObject(
							preferences.getString(PREFS_EVENTS, ""));
					JSONArray names = eventsObject.names() != null ? eventsObject
							.names() : new JSONArray();
					final int size = names.length();
					for (int index = 0; index < size; index++) {
						JSONArray eventArray = eventsObject.getJSONArray(names
								.getString(index));
						HashSet<ShopeliaEvent> events = new HashSet<ShopeliaEvent>(
								eventArray.length());
						final int eventArrayLength = eventArray.length();
						for (int indexEventArray = 0; indexEventArray < eventArrayLength; indexEventArray++) {
							ShopeliaEvent loaded = ShopeliaEvent
									.inflate(eventArray
											.getJSONObject(indexEventArray));
							events.add(loaded);
						}
	...
	private static void updateEvent(HashSet<ShopeliaEvent> events,
			ShopeliaEvent update) {
	...
	private FlushDelegate mPrivateFlushDelegate = new FlushDelegate() {
		...
		public boolean send(String uuid, String tracker, String action,
			...
			try {
				...
				HttpResponse response = ShopeliaRestClient.V1(context).post(
						Command.V1.Events.$, params);
	...
	public List<ShopeliaEvent> flush(String uuid, String trackerName,
			ShopeliaEvent[] events) {
		final long now = System.currentTimeMillis();
		ArrayList<ShopeliaEvent> notSent = new ArrayList<ShopeliaEvent>(
				events.length);
		HashMap<String, ArrayList<ShopeliaEvent>> sortedByAction = new HashMap<String, ArrayList<ShopeliaEvent>>();

		for (ShopeliaEvent event : events) {
			if (event.request > 0
					&& (event.sent_at == ShopeliaEvent.NEVER_SENT || event.sent_at
							+ mExpiryDelay <= now)) {
				if (!sortedByAction.containsKey(event.action)) {
					sortedByAction.put(event.action,
							new ArrayList<ShopeliaEvent>(events.length));
				}
	...
	private void sendAll(String uuid, String tracker,
			HashMap<String, ArrayList<ShopeliaEvent>> sortedByAction,
			ArrayList<ShopeliaEvent> notSent) {
		for (Map.Entry<String, ArrayList<ShopeliaEvent>> entry : sortedByAction
				.entrySet()) {
			if (!mFlushDelegate.send(uuid, tracker, entry.getKey(),
					entry.getValue())) {

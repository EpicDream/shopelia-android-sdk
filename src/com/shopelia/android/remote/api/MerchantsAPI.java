...
public class MerchantsAPI extends ApiController {
	...
	public static class OnRetrieveMerchantEvent extends
			OnResourceEvent<Merchant> {
	...
	public Merchant getMerchant(final String url) {
		...
		if (out == null && canFetchMerchants()) {
			ShopeliaRestClient.V1(getContext()).get(Command.V1.Merchants.$,
					null, new AsyncCallback() {

						@Override
						public void onComplete(HttpResponse httpResponse) {
							try {
								mMerchants = Merchant.inflate(new JSONArray(
										httpResponse.getBodyAsString()));
								saveMerchants(mMerchants);
								Merchant out = findMerchantByUrl(url);
								if (out != null) {
									getEventBus().post(
											new OnRetrieveMerchantEvent(out));
								}
							} catch (Exception e) {
	...
	public boolean update() {
		if (canFetchMerchants()) {
			ShopeliaRestClient.V1(getContext()).get(Command.V1.Merchants.$,
					new ParameterMap(), new AsyncCallback() {
						...
						public void onComplete(HttpResponse httpResponse) {
							...
							try {
								...
								saveMerchants(mMerchants);
								getEventBus()
										.post(new OnRetriveMerchantsEvent(
												mMerchants));
							} catch (Exception e) {
								if (Config.ERROR_LOGS_ENABLED) {
									e.printStackTrace();
	...
	private boolean canFetchMerchants() {
		return mPreferences.getLong(PREFS_LAST_UPDATE, 0) + DELAY_BEFORE_UPDATE < System
				.currentTimeMillis();
	}

	private void saveMerchants(List<Merchant> merchants) {
		SharedPreferences.Editor editor = mPreferences.edit();
		editor.putString(PREFS_MERCHANTS, JsonUtils.toJson(mMerchants)
				.toString());
	...
	private Merchant findMerchantByUrl(String url) {
		...
		for (Merchant merchant : mMerchants) {
			if (merchant.isValid()
					&& url.contains(merchant.uri.getHost().replace("www.", ""))) {
				return merchant;
			}
		}
		return null;

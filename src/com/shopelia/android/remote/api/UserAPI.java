...
public class UserAPI extends ApiController {
	...
	public static class OnAccountCreationSucceedEvent extends
			OnAddResourceEvent<User> {
	...
	private static final Class<?>[] sEventTypes = new Class<?>[] {
			OnAccountCreationSucceedEvent.class, OnAccountDestroyedEvent.class,
			OnSignInEvent.class, OnSignOutEvent.class,
	...
	public void createAccount(String tracker, User user, Address address,
			PaymentCard card) {

		JSONObject params = new JSONObject();

		try {
			params.put(Order.Api.USER,
					User.createObjectForAccountCreation(user, address, card));
			params.put(Order.Api.TRACKER, tracker);
		} catch (JSONException e) {
			fireError(null, null, e);
			return;
		}
		ShopeliaRestClient.V1(getContext()).post(Command.V1.Users.$, params,
				new JsonAsyncCallback() {

					@Override
					public void onComplete(HttpResponse response,
							JSONObject object) {
						if (object.has(User.Api.USER)
								&& object.has(User.Api.AUTH_TOKEN)) {
							User user = User.inflate(object
									.optJSONObject(User.Api.USER));
							UserManager.get(getContext()).login(user,
									object.optString(User.Api.AUTH_TOKEN));
							UserManager.get(getContext()).saveUser();
							if (user.addresses.size() > 0) {
								getEventBus()
										.post(new OnAccountCreationSucceedEvent(
												user));
							} else {
								fireError(response, null,
	...
	public void destroyUser(final long id) {
		...
		ShopeliaRestClient.V1(getContext()).delete(Command.V1.Users.User(id),
	...
	public void updateUser() {
		ShopeliaRestClient.V1(getContext())
				.get(Command.V1.Users.User(UserManager.get(getContext())
						.getUser().id), null, new AsyncCallback() {
					...
					public void onComplete(HttpResponse httpResponse) {
						if (httpResponse.getStatus() == 401) {
							...
							try {
								User user = User.inflate(new JSONObject(
										httpResponse.getBodyAsString())
										.getJSONObject(User.Api.USER));
								UserManager.get(getContext()).update(user);
								getEventBus().post(
										new OnUserRetrievedEvent(user));
								getEventBus().post(new OnUserUpdateDoneEvent());
							} catch (JSONException e) {
								onError(e);
							}
	...
	public void signIn(User user) {
		...
		ShopeliaRestClient.V1(getContext()).post(Command.V1.Users.SignIn.$,
				params, new AsyncCallback() {
					...
					public void onComplete(HttpResponse httpResponse) {
						...
						if (httpResponse.getStatus() == 200) {
							...
							fireError(httpResponse, ErrorInflater
									.inflate(httpResponse.getBodyAsString()),
									null);
						}
					}
	...
	public void signOut(final String email) {
		...
		ShopeliaRestClient.V1(getContext()).authenticate(getContext());
		ShopeliaRestClient.V1(getContext()).delete(Command.V1.Users.SignOut(),
				params, new AsyncCallback() {
